...
class PMMInventoryShift(ScriptStrategyBase):
    ...
    def get_candles_with_features(self):
        ...
        # Bollinger Bands
        candles_df.ta.bbands(length=self.candles_length, std=2, append=True)
    ...
    def update_multipliers(self):
        ...
        # === STEP 2: Bollinger Band-based adjustment ===
        price = candles_df["close"].iloc[-1]
        upper = candles_df[f"BB_UPPER_{self.candles_length}"].iloc[-1]
        lower = candles_df[f"BB_LOWER_{self.candles_length}"].iloc[-1]

        band_width = upper - lower
        if band_width == 0:
            position = 0.5  # Avoid division by zero, assume neutral
        else:
            position = (price - lower) / band_width


        # Adjust spreads based on position in the band
        if position >= 0.66:
            # Price near upper band → Expect reversion down
            self.ask_spread *= Decimal("0.75")  # tighter ask
            self.bid_spread *= Decimal("1.25")  # wider bid
        elif position <= 0.33:
            # Price near lower band → Expect reversion up
            self.ask_spread *= Decimal("1.25")  # wider ask
            self.bid_spread *= Decimal("0.75")  # tighter bid
        # else: Neutral zone, keep as is
